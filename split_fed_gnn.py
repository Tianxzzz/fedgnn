...
def setup():
    ...
    for m in range(50):
            ...
            for epoch_i in range(2):
                ...
                for batch in server_train_dataloader:
                    ...
                    graph_encoding = server_model(
                        Data(x=graph_encoding,edge_index=data['train']['edge_index'].to('cuda'),
                             edge_attr=data['train']['edge_attr'].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to('cuda'))
                    )  # N x B x L x F
                    if epoch_i == 1:
        ...
        print(id, log.format(m, np.mean(mse),np.mean(rmse), np.mean(mae),np.mean(mape),
                             (t2 - t1), flush=True))
        torch.save(server_model.state_dict(), "params_avg/" + "server_epoch_" + str(m) + "_" + str(round(np.mean(mse), 4)) + ".pth")
        torch.save(client_model.state_dict(),
                   "params_avg/" + "client_epoch_" + str(m) + "_" + str(round(np.mean(mse), 4)) + ".pth")
