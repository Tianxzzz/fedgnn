...
def setup():
    ...
    device='cuda'
    data = load_dataset(dataset_name='PEMS-BAY')
    dataset = data
    ...
    for m in range(10):
        ...
        # index = random.sample(range(0, num_clients), )
        # for i in index:
        #     adp[[i], :] = 0
        #     adp[:, [i]] = 0
        ...
        with torch.no_grad():
            ...
            for batch in server_val_dataloader:
                ...
                graph_encoding = server_model(
                    Data(x=graph_encoding, edge_index=val_edge_index.to('cuda')))
                # print(graph_encoding.shape)torch.Size([207, 48, 1, 64])
                updated_graph_encoding.append(graph_encoding.detach().clone().cpu())
        updated_graph_encoding = torch.cat(updated_graph_encoding, dim=1)  # N x B x L x F
        for client_i, client_params in enumerate(client_params_list):
        ...
        print(client_log)
        torch.save(server_model.state_dict(), "GATGN/PEMS-bay/MSEdrop0/" + "server_epoch_" + str(m) + "_" + str(client_log['rmse']) + ".pth")
        torch.save(client_model.state_dict(), "GATGN/PEMS-bay/MSEdrop0/" + "client_epoch_" + str(m) + "_" + str(client_log['rmse']) + ".pth")
