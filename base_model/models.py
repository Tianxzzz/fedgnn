...
class TCN(nn.Module):
    def __init__(self,  in_dim=2,out_dim=12,residual_channels=32,dilation_channels=32,skip_channels=256,end_channels=512,kernel_size=2):
        ...
        self.gate_convs=nn.Conv2d(in_channels=in_dim,
                                         out_channels=dilation_channels,
                                         kernel_size=(1, 2), dilation=1)
...
class graphmodel(nn.Module):
    ...
    def __init__(self,num_nodes):
        ...
        self.gat=GATv2Conv(48*32,48*32)
        self.linear=nn.Linear(48*11*32,48*32)
    ...
    def forward(self,x):
        ...
        # print(x.shape)
        res=x
        x=x.reshape(res.shape[1],res.shape[0]*res.shape[2]*res.shape[3])
        x=self.linear(x)
        # _,edge_index=self.gat(x,edge_index,return_attention_weights=True)

        y=x.reshape(res.shape[1],res.shape[0],res.shape[3])
        y=y.transpose(0,1)
        y=self.gcn1(y,edge_index)
        y=self.gcn2(y,edge_index)
        # y = y.reshape(res.shape[0],res.shape[1], res.shape[3])
