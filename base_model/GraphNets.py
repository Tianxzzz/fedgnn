...
class GATGN(nn.Module):
    def __init__(self, node_input_size, edge_input_size, global_input_size,
        ...
        self.net = nn.ModuleList(self.net)
        self.node_out_net = nn.Linear(last_node_input_size, node_output_size)
        self.gat = GATConv(48 * 64, 48 * 32)
        self.gat1 = GATv2Conv(48 * 32, 48 * 32)
        self.BN1=nn.BatchNorm1d(48 * 32)
    ...
    def forward(self, data,epoch):
        ...
        gatoutput1, edge = self.gat(x=gatinput, edge_index=edge_index, return_attention_weights=True)
        edge_index = edge[0]
        edge_attr = edge[1]
        gatoutput1 = F.relu(gatoutput1)
        gatoutput1=self.BN1(gatoutput1)

        gatoutput, edge = self.gat1(x=gatoutput1,edge_index= edge_index, return_attention_weights=True)
        gatoutput = F.relu(gatoutput)
        gatoutput = self.BN2(gatoutput)
        # print(x.shape,gatoutput.shape)
        edge_index = edge[0]
        edge_attr = edge[1]
        if self.training:
            ...
            index = np.random.randint(0, 207, size=51)
            for i in index:
                adj[[i], :] = 0
                adj[:, [i]] = 0
        ...
        gatoutput=torch.cat((gatoutput1.reshape(x.shape[0],x.shape[1],1,32),gatoutput.reshape(x.shape[0],x.shape[1],1,32)),dim=-1)
        x = gatoutput
        edge_attr = edge_attr.unsqueeze(-1).unsqueeze(-1)
        #
        # # # batch = batch.to('cuda')
